...
namespace Azure.ResourceManager.AppService
{
    ...
    public partial class WebSiteSlotCollection : ArmCollection, IEnumerable<WebSiteSlotResource>, IAsyncEnumerable<WebSiteSlotResource>
    {
        ...
        {
            async Task<Page<WebSiteSlotResource>> FirstPageFunc(int? pageSizeHint)
            {
                using var scope = _webSiteSlotWebAppsClientDiagnostics.CreateScope("WebSiteSlotCollection.GetAll");
                scope.Start();
                try
                {
                    var response = await _webSiteSlotWebAppsRestClient.ListSlotsAsync(Id.SubscriptionId, Id.ResourceGroupName, Id.Name, cancellationToken: cancellationToken).ConfigureAwait(false);
                    return Page.FromValues(response.Value.Value.Select(value => new WebSiteSlotResource(Client, value)), response.Value.NextLink, response.GetRawResponse());
                }
                catch (Exception e)
                {
                    scope.Failed(e);
            ...
            async Task<Page<WebSiteSlotResource>> NextPageFunc(string nextLink, int? pageSizeHint)
            {
                using var scope = _webSiteSlotWebAppsClientDiagnostics.CreateScope("WebSiteSlotCollection.GetAll");
                scope.Start();
                try
                {
                    var response = await _webSiteSlotWebAppsRestClient.ListSlotsNextPageAsync(nextLink, Id.SubscriptionId, Id.ResourceGroupName, Id.Name, cancellationToken: cancellationToken).ConfigureAwait(false);
                    return Page.FromValues(response.Value.Value.Select(value => new WebSiteSlotResource(Client, value)), response.Value.NextLink, response.GetRawResponse());
                }
                catch (Exception e)
                {
        ...
        {
            Page<WebSiteSlotResource> FirstPageFunc(int? pageSizeHint)
            {
                using var scope = _webSiteSlotWebAppsClientDiagnostics.CreateScope("WebSiteSlotCollection.GetAll");
                scope.Start();
                try
                {
                    var response = _webSiteSlotWebAppsRestClient.ListSlots(Id.SubscriptionId, Id.ResourceGroupName, Id.Name, cancellationToken: cancellationToken);
                    return Page.FromValues(response.Value.Value.Select(value => new WebSiteSlotResource(Client, value)), response.Value.NextLink, response.GetRawResponse());
                }
                catch (Exception e)
                {
            ...
            Page<WebSiteSlotResource> NextPageFunc(string nextLink, int? pageSizeHint)
            {
                using var scope = _webSiteSlotWebAppsClientDiagnostics.CreateScope("WebSiteSlotCollection.GetAll");
                scope.Start();
                try
                {
                    var response = _webSiteSlotWebAppsRestClient.ListSlotsNextPage(nextLink, Id.SubscriptionId, Id.ResourceGroupName, Id.Name, cancellationToken: cancellationToken);
                    return Page.FromValues(response.Value.Value.Select(value => new WebSiteSlotResource(Client, value)), response.Value.NextLink, response.GetRawResponse());
                }
                catch (Exception e)
                {
